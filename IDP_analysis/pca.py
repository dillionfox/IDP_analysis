...
def plot_Kmeans_PCA(B, ind_list, PC_labels, kmeansoutput, outdir, name_mod, scores, ros_frames):
	"""
	...
	np.savetxt(outdir+'PCA_kmeans_B' + name_mod + '.npy',np.array(B))
	return None
...
def run_PCA(EED,Rg,SASA,Asph,outdir,name_mod,mode,scores,trajname,ros_frames,calcs):
	"""
	...
	norm_EED =  norm_shift(EED)
	norm_Rg =   norm_shift(Rg)
	norm_SASA = norm_shift(SASA) 
	norm_Asph = norm_shift(Asph)
	...
	ind_list = extract_PCA(pca, PC_labels, mode, outdir, name_mod)

	#---Do initial K-means clustering to determine number of clusters
	nK = compute_Kmeans(B,mode)

	#---Use optimum number of clusters for k-means
	kmeans=KMeans(n_clusters=nK)
	if len(ind_list) > 1:
		kmeansoutput=kmeans.fit(np.array([B[:,0],B[:,1]]).T)

		#---Plot top 2 PCA clusters colored by kmeans
		plot_Kmeans_PCA(B, ind_list, PC_labels, kmeansoutput, outdir, name_mod, scores, ros_frames)
		k = np.array(kmeansoutput.labels_)
		cluster_centroids(B,k,calcs,mode,scores,outdir,name_mod,trajname) 
...
def cluster_centroids(B,k,calcs,mode,scores,outdir,name_mod,trajname):
	"""
	...
	if 'PCA' not in calcs:
		ind_list = extract_inds(outdir+'PCA'+name_mod+ '.txt')

	N = int(max(k))
	centroids = []
	labels = []
	coors = []
	B = np.array([B[:,0], B[:,1]]).T
	for n in range(N+1):
		b = B[np.where(k == n)]
